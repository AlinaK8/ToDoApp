...
function editToDoView(id, title, text) {
    
    let tableData = document.querySelector(`[data-id="${id}"]`);
    tableData.firstElementChild.innerHTML += title;
    tableData.firstElementChild.nextElementSibling.innerHTML += text;
}
...
function setFormtoEdit(id) {
    ...
    for (let i = 0; i < todosStorage.todos.length; i++) {
        console.log(todosStorage.todos[i]);
        if (todosStorage.todos[i].id === id ) {
